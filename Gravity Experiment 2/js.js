...
$(document).ready(function(){
	...
	function paint()
	{
        ...
        var gravity =  planet1.r *(planet1.r*20/dist)/1000;
        ...
        //AUTOROTACION
        if( dist<atm){
            ship.rot += (((0.2*sum) * (1-(dist-(planet1.r))/(atm-planet1.r)))/(16/1000))*frameDuration/1000;
        }
        ...
        //FRICTION
        if( dist<(atm)){
            ship.velx *= (1-(((0.009*fr)/(16/1000))*frameDuration/1000));
            ship.vely *= (1-(((0.009*fr)/(16/1000))*frameDuration/1000));
        }
        ...
        ctx.beginPath();
        ctx.arc(planet1.x+despX,planet1.y+despY,atm,0,2*Math.PI);
        ctx.fillStyle = '#blue';
        ctx.globalAlpha=0.1;
        ctx.fill();
        ctx.globalAlpha=1;
        
        drawCircle(planet1.x+despX,planet1.y+despY,planet1.r);
        ...
        ctx.beginPath();
        ctx.moveTo(ship.x+despX,ship.y+despY);
        ctx.lineTo(ship.x+despX-gravity*100*Math.cos(ang1),ship.y+despY-gravity*100*Math.sin(ang1));
        ctx.stroke();
    ...
    function drawShip(ship){
        ...
        ctx.beginPath();
        ctx.moveTo((ship.x+despX)+(ship.pointsX[0]*Math.cos(ship.rot)-ship.pointsY[0]*Math.sin(ship.rot)), (ship.y+despY)+(ship.pointsX[0]*Math.sin(ship.rot)+ship.pointsY[0]*Math.cos(ship.rot)));
        for(i=1;i<ship.pointsX.length;i++){
            ctx.lineTo((ship.x+despX)+(ship.pointsX[i]*Math.cos(ship.rot)-ship.pointsY[i]*Math.sin(ship.rot)), (ship.y+despY)+(ship.pointsX[i]*Math.sin(ship.rot)+ship.pointsY[i]*Math.cos(ship.rot)));
        }
        ctx.closePath();
        ctx.fillStyle = "white";
        ctx.fill();
    ...
    function drawLines(){
        ...
        for(i=0;i<w;i+=w/4){
            ctx.rect(i+despX%(w/4),0,0,h);
            ctx.stroke();
        }
        for(i=0;i<=h+10;i+=h/2){
            ctx.rect(0,i+despY%(h/2),w,0);
            ctx.stroke();
        }
